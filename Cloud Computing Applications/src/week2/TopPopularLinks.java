...
// <<< Don't Change
	...
	@Override
        public void map(Text key, Text value, Context context) throws IOException, InterruptedException {
            ...
            topPages.add(new Pair<>(count, node));

            if (topPages.size() > N) {
                topPages.remove(topPages.first());
            }
        }

        @Override
        protected void cleanup(Context context) throws IOException, InterruptedException {
            for (Pair<Integer, Integer> pair : topPages) {
                Integer[] list = {pair.second, pair.first};
                context.write(NullWritable.get(), new IntArrayWritable(list));
            }
        }
    }
	...
	@Override
        public void reduce(NullWritable key, Iterable<IntArrayWritable> values, Context context) throws IOException, InterruptedException {
            for (IntArrayWritable val : values) {
                ...
                Integer node = list[0].get();
                Integer count = list[1].get();
    ...
    static class Pair<A extends Comparable<? super A>,
